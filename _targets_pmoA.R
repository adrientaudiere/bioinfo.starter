...
if (tar_active()) {
  log_start(
    path = "data/data_final/autometric_log_pmoA.txt",
    seconds = 1
  )
}
...
seq_len_min <- 200
fw_primer_sequences <- "GGNGACTGGGACTTCTGG"
rev_primer_sequences <- "CCGGMGCAACGTCYTTACC"
n_threads <- 3
...
tar_plan(
  ...
  tar_target(data_raw, {
    cutadapt
    list_fastq_files(path = here::here("data/data_intermediate/pmoA_seq_wo_primers/"))
  }),
  ...
  tar_target(
    ...
    filter_trim(
      output_fw = paste(
        getwd(),
        here("/data/data_intermediate/pmoA_filterAndTrim_fwd"),
        sep = ""
      ),
      output_rev = paste(
        getwd(),
        here("/data/data_intermediate/pmoA_filterAndTrim_rev"),
        sep = ""
      ),
      fw = data_fnfs,
      rev = data_fnrs,
  ...
  ### Build a a table of ASV x Samples Paired-end
  tar_target(seq_tab_Pairs, makeSequenceTable(merged_seq)),

  ### Build a a table of ASV x Samples forward only
  tar_target(seq_tab_Fw, makeSequenceTable(ddF)),
  ...
  ##> Create the phyloseq object without taxonomy (no reference database for pmoA)
  ...
  tar_target(track_sequences_samples_clusters, track_wkflow(
    list(
      ...
      "Forward wo primers (pmoA)" = unlist(list_fastq_files(here::here("data/data_intermediate/pmoA_seq_wo_primers/"), paired_end = FALSE)),
  ...
  tar_target(
    quality_raw_seq,
    fastqc_agg(fastq_files_folder, qc.dir = here("data/data_final/quality_fastqc/pmoA_raw_seq/"), multiqc=TRUE)
  ),
  ### After cutadapt
  tar_target(
    quality_seq_wo_primers, {cutadapt
    fastqc_agg(here("data/data_intermediate/pmoA_seq_wo_primers/"), qc.dir = here("data/data_final/quality_fastqc/pmoA_seq_wo_primers/"), multiqc=TRUE)
  }),
  ### After filtering and trimming (separate report for forward and reverse)
  tar_target(
    quality_seq_filtered_trimmed_FW,
    fastqc_agg(here(filtered[[1]]), qc.dir = here("data/data_final/quality_fastqc/pmoA_filterAndTrim_fwd/"), multiqc=TRUE)
  ),
  tar_target(
    quality_seq_filtered_trimmed_REV,
    fastqc_agg(here(filtered[[1]]), qc.dir = here("data/data_final/quality_fastqc/pmoA_filterAndTrim_rev/"), multiqc=TRUE)
  )
)

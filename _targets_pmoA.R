...
tar_plan(
  ...
  tar_target(data_raw, {
    cutadapt
    list_fastq_files(
      path = here::here("data/data_intermediate/pmoA_seq_wo_primers/")
    )
  }),
  ...
  tar_target(
    ...
    track_wkflow(
      list(
        ...
        "Forward wo primers (pmoA)" = unlist(list_fastq_files(
          here::here("data/data_intermediate/pmoA_seq_wo_primers/"),
          paired_end = FALSE
        )),
  ...
  tar_target(
    ...
    fastqc_agg(
      fastq_files_folder,
      qc.dir = here("data/data_final/quality_fastqc/pmoA_raw_seq/"),
      multiqc = TRUE
    )
  ),
  ### After cutadapt
  tar_target(
    quality_seq_wo_primers,
    {
      cutadapt
      fastqc_agg(
        here("data/data_intermediate/pmoA_seq_wo_primers/"),
        qc.dir = here("data/data_final/quality_fastqc/pmoA_seq_wo_primers/"),
        multiqc = TRUE
      )
    }
  ),
  ...
  tar_target(
    ...
    fastqc_agg(
      here(filtered[[1]]),
      qc.dir = here("data/data_final/quality_fastqc/pmoA_filterAndTrim_fwd/"),
      multiqc = TRUE
    )
  ),
  tar_target(
    ...
    fastqc_agg(
      here(filtered[[1]]),
      qc.dir = here("data/data_final/quality_fastqc/pmoA_filterAndTrim_rev/"),
      multiqc = TRUE
    )
  )
)

...
tar_plan(
  ...
  tar_target(data_raw, {
    cutadapt
    list_fastq_files(
      path = here::here("data/data_intermediate/seq_wo_primers/")
    )
  }),
  ...
  tar_target(
    ...
    fastqc_agg(
      fastq_files_folder,
      qc.dir = here("data/data_final/quality_fastqc/raw_seq/"),
      multiqc = TRUE
    )
  ),
  ### After cutadapt
  tar_target(
    quality_seq_wo_primers,
    {
      cutadapt
      fastqc_agg(
        here("data/data_intermediate/seq_wo_primers/"),
        qc.dir = here("data/data_final/quality_fastqc/seq_wo_primers/"),
        multiqc = TRUE
      )
    }
  ),
  ...
  tar_target(
    ...
    fastqc_agg(
      here(filtered[[1]]),
      qc.dir = here("data/data_final/quality_fastqc/filterAndTrim_fwd/"),
      multiqc = TRUE
    )
  ),
  tar_target(
    ...
    fastqc_agg(
      here(filtered[[1]]),
      qc.dir = here("data/data_final/quality_fastqc/filterAndTrim_rev/"),
      multiqc = TRUE
    )
  ),

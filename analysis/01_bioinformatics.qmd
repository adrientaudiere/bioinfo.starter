...
/home/adrien/Bureau/bioinfo.starter_solace/data/data_raw/refseq/raxml-ng2 --all --msa seqs_mfre_for_mafft_MAFFTED.fasta --model DNA --tree pars{10} --bs-trees 1000 --prefix raxml_otu_mfre_og
```
...
```{sh}
/home/adrien/Bureau/bioinfo.starter_solace/data/data_raw/refseq/iqtree3/bin/iqtree3 -s /home/adrien/Bureau/bioinfo.starter_solace/data/data_intermediate/seqs_mfre_for_mafft_MAFFTED.fasta -B 1000
```

```{r}
library(ape)
iqtree_muco_og <- read.tree("/home/adrien/Bureau/bioinfo.starter_solace/data/data_intermediate/seqs_mfre_for_mafft_MAFFTED.fasta.contree")
outgroups <- iqtree_muco_og$tip.label[c(grep("JX301", iqtree_muco_og$tip.label), grep("OUT_", iqtree_muco_og$tip.label))]
iqtree_muco_og <- root(iqtree_muco_og, outgroup=outgroups)

nodes_color <- rep("green", length(iqtree_muco_og$tip.label))
names(nodes_color) <- iqtree_muco_og$tip.label
nodes_color[grepl("Taxa",names(nodes_color))] <- "gray20"
nodes_color[names(nodes_color) %in% outgroups] <- "red"

plot(iqtree_muco_og, show.node.label = TRUE, tip.color = nodes_color)
```


```{r}
ggtree(iqtree_muco_og) + 
  geom_tiplab(aes(color=case_when(grepl("Taxa",label) ~ "mfre",
                                  grepl("OUT",label) ~ "outgroup",
                                  grepl("JX",label) ~ "outgroup",
                                  .default = "refseq")),
